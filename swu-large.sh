...
update() {
	...
	cat $here/deploy/venus/images/$machine/$swu-large-$machine.swu | ssh $sshargs root@$host /opt/victronenergy/swupdate-scripts/check-updates.sh -swu file:///dev/stdin
}
